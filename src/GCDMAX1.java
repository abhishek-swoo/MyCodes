...
class GCDMAX1 {
    ...
    //Solution !!
    SegmentTreeRMQ strmq;
    void solution()
    {
        int n=inti(),k=inti();
        int arr[]=arri(n);
        strmq=new SegmentTreeRMQ(arr, n);
//        out.println(strmq.query(2, 3));
        int high=n+1,low=0;
        while(high>low+1){
            int mid=(high+low)/2;
            if(solve(mid, n,k)){
...
//                out.println(0);
    ...
    boolean solve(int mid,int n,int k){
        for(int i=0;i<=n-mid;i++){
//            out.println(strmq.query(i-k, i));
            int gcd=strmq.query(i, i+mid-1);
            if(gcd>=k)
                return true;
        }
        return false;
