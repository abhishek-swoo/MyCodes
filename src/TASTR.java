...
class TASTR {
    ...
    //Solution !!
    void solution() {
        String s1 = stri(), s2 = stri();
        long subS1=distinctSubstring(s1);
        long subS2=distinctSubstring(s2);
        String s3=s1+"$"+s2;
        long subS3=distinctSubstring(s3)-(1L*s1.length()+1)*(1L*s2.length()+1);
        long ans=2*(subS3)-subS1-subS2;
        out.println(ans);
    }

    long distinctSubstring(String s) {
