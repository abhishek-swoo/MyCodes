...
class TAXI {
    ...
    void solution() {
        for (int tn = inti(); tn > 0; tn--) {
            int p = inti(), t = inti(), s = inti(), c = inti();
            List<Edge> es = new ArrayList<>();
            int person[][] = new int[p][2];
            int taxi[][] = new int[t][2];
            for (int i = 0; i < p; i++) {
                person[i][0] = inti();
                person[i][1] = inti();
            }
            ...
            for (int i = 0; i < p; i++) {
                for (int j = 0; j < t; j++) {
                    int d = Math.abs(person[i][0] - taxi[j][0]) + Math.abs(person[i][1] - taxi[j][1]);
                    d *= 200;
                    double time = d / s;
                    if (time > c) {
                        continue;
                    }
                    es.add(new Edge(i+1, j+p+1, 1));
                }
            }
            for(int i=0;i<p;i++){
                es.add(new Edge(0,i+1 , 1));
            }
            for(int i=0;i<t;i++){
                es.add(new Edge(i+1+p,p+t+1 , 1));
            }
            Edge g[][]=compileWD(p+t+2, es);
            out.println(maximumFlowDinic(g, 0, p+t+1));
        }
    }
