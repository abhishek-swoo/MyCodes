...
class TRT {
    ...
    void solution()
    {
        ...
        int[] cow=arri(n);
        dp=new long[n][n];
        long val=Math.max(cow[0]+treat(cow, 1, n-1, n), cow[n-1]+treat(cow, 0, n-2, n));
        out.println(val);
    }
    long dp[][];
    long treat(int[] cow, int start, int end, int n){
        if(n-end > n-start)
            return 0;
        if(dp[start][end]!=0)
            return dp[start][end];
        return dp[start][end]=Math.max( (cow[start]*(n - end + start))+treat(cow, start+1, end, n),(cow[end]*(n - end + start))+treat(cow, start, end-1, n));
    }
    //------->ends !!
