...
class AGGRCOW {
    ...
    void solution()
    {
        for(int tn=inti();tn>0;tn--){
            ...
            int arr[]=arri(n);
            Arrays.sort(arr);
            out.println(BinarySearch(arr, c));
        }
//        int[] arr=arri(5);
//        out.println(BinarySearch(arr, 3));
    }
    
    boolean func(int num,int arr[],int c){
    ...
    int BinarySearch(int arr[],int c){
        int low=0,high=arr[arr.length-1],max=-1,mid;
        while(high>low){
            mid=(low+high)/2;
            if(func(mid, arr, c)){
                if(mid>max)
