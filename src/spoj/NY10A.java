    ...
    void solution()
    {
        for(int tn=inti();tn>0;tn--){
            ...
            for(int i=0;i<st.length()-2;i++){
                if(hh.containsKey(st.substring(i,i+3))){
                    u=hh.get(st.substring(i,i+3));
                    hh.replace(st.substring(i, i+3), u, u+1);
                }
                else{
                    hh.put(st.substring(i, i+3), 1);
                }
            }
            if(!hh.containsKey("HHH"))
                hh.put("HHH", 0);
            if(!hh.containsKey("TTT"))
                hh.put("TTT", 0);
            if(!hh.containsKey("TTH"))
                hh.put("TTH", 0);
            if(!hh.containsKey("THT"))
                hh.put("THT", 0);
            if(!hh.containsKey("THH"))
                hh.put("THH", 0);
            if(!hh.containsKey("HTT"))
                hh.put("HTT", 0);
            if(!hh.containsKey("HTH"))
                hh.put("HTH", 0);
            if(!hh.containsKey("HHT"))
                hh.put("HHT", 0);
            out.println(hh.get("TTT")+" "+hh.get("TTH")+" "+hh.get("THT")+" "+hh.get("THH")+" "+hh.get("HTT")+" "+hh.get("HTH")+" "+hh.get("HHT")+" "+hh.get("HHH"));
        }
    }
    //------->ends !!
